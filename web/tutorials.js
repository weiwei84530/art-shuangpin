  ...
  },
  {
    ...
    steps: [
      ...
      { keys: ['m','a','8'], screen: { comp: [['我','s'],['馬','s']] },
        cap: '接著打「馬」。ㄇㄚˇ 是三聲，但這次用右手的 <kbd>8</kbd>：以 <kbd>5</kbd>/<kbd>6</kbd> 之間為軸<b>左右鏡像</b>，<kbd>0</kbd>＝一聲、<kbd>9</kbd>＝二聲、<kbd>8</kbd>＝三聲、<kbd>7</kbd>＝四聲。輕聲<b>只有左手的 <kbd>5</kbd></b>——<kbd>6</kbd> 另有工作，見〈數字鍵的兩種身分〉。' },
      ...
      { keys: [], screen: {  },
        cap: '整理：<b>不打數字</b>＝一聲＋輕聲（最常用，能省就省）；<kbd>1</kbd>/<kbd>0</kbd>＝嚴格只要一聲；<kbd>2</kbd><kbd>3</kbd><kbd>4</kbd>（或 <kbd>9</kbd><kbd>8</kbd><kbd>7</kbd>）＝精確聲調；<kbd>5</kbd>＝只要輕聲（輕聲沒有右手鏡像鍵）。<b>實務上幾乎不用打一聲和輕聲</b>——不打數字本來就涵蓋這兩個，明打只在你要<b>排除另一個</b>時才有意義。' }
    ]
  },
  {
    id: 'settle', stage: '入門', title: '定案與上屏：空白鍵是定案鍵',
    steps: [
      ...
      { keys: ['Space'], screen: { comp: [['中','s'],['文','s'],[' ','s']] },
        cap: '這時<b>已經沒有待定的音</b>，空白鍵就打出一個<b>普通的半形空白</b>。<b>底線還在</b>——整段仍然沒有送出去。' },
      { keys: ['Backspace'], screen: { comp: [['中','s'],['文','s']] },
        cap: '那個空白就是一般的組字串內容：<kbd>⌫</kbd> 刪得掉，游標也走得過去。所以空白鍵的規則只有一條：<b>有東西可定案就定案，沒有就打一個空白</b>。' },
      { keys: ['Enter'], screen: { text: '中文', comp: [] },
        cap: '<b>上屏只有兩種方式</b>：<kbd>Enter</kbd>，以及點到別的地方（失焦）。<kbd>Esc</kbd> 則是整段丟掉。' },
      { keys: [], screen: {  },
        cap: '<b>沒有任何自動上屏的界線</b>——標點不會、空白不會、寫多長都不會。整段隨時都能回頭改字，直到你按 <kbd>Enter</kbd> 為止。' },
      { keys: [], screen: {  },
        cap: '<kbd>Enter</kbd> <b>不會丟掉任何東西</b>：還是注音的部分（例如只按了 <kbd>n</kbd> 的 ㄋ）會原樣送出去，不會憑空消失。詳見〈打注音符號本身〉那一課。' }
    ]
  },
  {
  ...
  },
  {
    ...
    steps: [
      { keys: ['n','c'], screen: { comp: [['ㄋㄧㄠ','p']] },
        cap: '有時候要輸出注音符號本身（標讀音、教小朋友）。<kbd>n</kbd><kbd>c</kbd>＝ㄋㄧㄠ，而詞庫裡<b>只有 ㄋㄧㄠˇ 和 ㄋㄧㄠˋ</b>，沒有一聲也沒有輕聲。' },
      { keys: ['Space'], screen: { comp: [['ㄋ','s'],['ㄧ','s'],['ㄠ','s']] },
        cap: '空白鍵沒有字可以定案，就把<b>注音本身</b>定案成符號留在組字串裡——可以和中文混排、用游標編輯、<kbd>⌫</kbd> 逐符號刪。<kbd>f</kbd> 也一樣：ㄈㄛ 只有二聲的「佛」，所以 <kbd>f</kbd>＋空白得到 ㄈ，要「佛」請打 <kbd>f</kbd><kbd>2</kbd>。' },
      { keys: ['k'], screen: { comp: [['ㄋ','s'],['ㄧ','s'],['ㄠ','s'],['ㄎ','p']] },
        cap: '第二條路更直接：<b>畫面上還是注音的時候按 <kbd>Enter</kbd>，就原樣送出</b>。<kbd>k</kbd> 顯示 ㄎ——它其實是個完整的音節 ㄎㄜ，按空白鍵會變成字。' },
      { keys: ['Enter'], screen: { text: 'ㄋㄧㄠㄎ', comp: [] },
        cap: '<kbd>Enter</kbd> 送出的就是<b>畫面上的東西</b>：ㄋㄧㄠ 和 ㄎ 都原樣出去。要「科」就先按空白鍵或聲調鍵讓它成字，再 <kbd>Enter</kbd>。' },
      { keys: [], screen: {  },
        cap: '所以 <kbd>Enter</kbd> <b>不會丟掉任何東西</b>。以前打到一半的音節（只按了 <kbd>n</kbd> 的 ㄋ）按 <kbd>Enter</kbd> 會憑空消失，現在會照樣送出去。' }
    ]
  }
];
