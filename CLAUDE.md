...
- **中英提示氣泡（2026-09-08）**：在游標旁邊閃一張小卡片顯示「中」或「英」，0.55 秒後淡出。**兩個平台同一個東西、同兩個時機**：Shift 單獨輕按，以及焦點落進可輸入的文字區（per-app 記憶換掉模式卻沒有任何提示的那一刻）。**沒有系統指示器可借**——macOS 對輸入法的內部切換不顯示任何東西，Windows（實測 Win 11 26200）也不顯示。Windows 是 `ime\SampleIME\ModeIndicator.{h,cpp}`（28×24 px @96dpi、11pt、ClearType、`CS_DROPSHADOW`，位置取自選取範圍的 `GetTextExt`，讀不到退回系統 caret 再退回滑鼠）；macOS 是 `mac/src/ArtModeHUD.mm`（30×26 pt、15pt，讀不到直接退回滑鼠）。**重閃時一律先收掉視窗、同步重畫再顯示**，否則會先看到上一次的字。**Shift 那次一定要在改組字串之前量游標（2026-09-09）**：`switchLanguage` 會補分隔空白、`_SyncComposer` 在同一個 edit session 裡重寫組字串，之後再問 Chromium 就會拿到退化矩形（呼叫本身不失敗）。**焦點那次的等待是 30/80/160 ms（2026-09-09 從 30/80/160/300 縮短）**——每個宿主都要等完才顯示，因為答得對的宿主答案從頭到尾一樣；**游標矩形必須有高度才算數（2026-09-09）**：寬 0 正常（游標本來就是一條線），高 0 不是游標——桌面的 `SysListView32` 與 Win11 工作列的 InputSite 都吃 type-ahead、都不標 keyboard-disabled，`GetTextExt` 照樣回 S_OK，回的是工作區右下角一個高度 0 的假矩形。macOS 的 `NSIsEmptyRect` 早就擋掉這一類。
...
- 2026-09-09：**有底線時按 Shift 不顯示氣泡（使用者在 Chrome 回報），VERSION 升到 0.9.0**。
  `_HandleShiftTap` 原本的順序是**先改組字串、再量游標**：`switchLanguage` 補分隔空白、`_SyncComposer`
  在**同一個 edit session** 裡把組字串重寫，之後才問 `GetTextExt`。改成**進函式先量、切完模式再用那個矩形顯示**
  （多一個 `_MeasureCaretUnderLock`，`_FlashModeIndicatorUnderLock` 取代掉）。位置最多差一個分隔空白的寬度。
  **推論又錯了一次，log 又救了一次**：我猜是 `TS_E_NOLAYOUT`，但整場 133 行 log **每一次 `GetTextExt` 都是
  `S_OK`**——失敗的不是呼叫，是回來的矩形（這個宿主會回 `(0,-993,0,-993)` 這種零高度答案），
  被 2026-09-09 新加的高度守衛擋掉。**教訓同 09-08 那條：位置類的 bug 先開 log 再改。**
  修完後拆掉那兩行診斷 log。
  **macOS 沒跟（使用者裁示）**：`ArtInputController.mm` 的 Shift 分支也是「先 `syncWithResult:` 再量」，
  但那邊沒有 document lock、`setMarkedText:` 是獨立呼叫，現行註解也主張切換後取樣是安全的，
  **沒有任何實測說它壞了**，所以不盲改。這個未解問題寫進 `mac/docs/NOTES.md` 的〈What to check first〉第 11 條，
  含「若真的壞了要怎麼修」。共用的 `core/`／`engine/` 這一輪零改動，marker 已推進到 `cc0baea`＋0.9.0。

- 2026-09-09：**焦點氣泡的等待從最久 570 ms 縮短成 270 ms（`kFlashRetryMs` 去掉最後一段 300）**，
  以及**兩個被實測否決的版本（勿再實作）**。使用者回報「有時候會停頓一下」，根因是輪詢的方向：
  等待存在只為了 Chromium 會回舊座標，而**答得對的宿主答案從頭到尾一樣、只能等滿才敢採信**，
  所以記事本這種宿主每次都付滿 30+80+160+300＝570 ms。
  **試過且撤銷的兩版**：(a) 偵測到「答案與這個 TIP 實例上次量到的矩形完全相同」＝宿主回舊值，
  就**什麼都不顯示**；(b) 同一個偵測，但回音時**照舊等**、非回音時立刻顯示（本機量到 30–141 ms）。
  兩版都在 **Chrome 實測不如預期**（使用者裁示），共同的問題是那個「回音」判斷在真實瀏覽器裡不可靠——
  它假設宿主回的舊值一定等於**我們自己上次量到的**那一個，實際上宿主的舊值可能更新（使用者在原欄位打過字、
  或游標被移動過），判斷就翻成「這是新答案」而立刻顯示在**上一個欄位**旁。
  **最後採用的是最單純的那條**：不加任何啟發式，只把時間表縮短一段，所有宿主一律快一倍。
  **已知代價（使用者知情）**：Chromium 若某次改口比 270 ms 晚，那一次的氣泡會落在上一個欄位旁邊
  （＝09-09 修掉的症狀，變罕見但沒有消失）。要再調就只有這一個旋鈕，兩個方向的代價寫在 `kFlashRetryMs` 上面。
  驗證：兩架構重建、`ctest` x64／x86 各 178 全過，打包並 install.ps1 裝到本機；行為由使用者實測。
- 2026-09-09：**點桌布／工作列會在螢幕右下角冒出中英氣泡（使用者回報），VERSION 升到 0.8.8**。
  改動只有 `ime/SampleIME/SampleIME.cpp` 一處判斷，`core/`／`engine/`／`mac/src/` 零改動。
  **不是猜的，是量的**（照 09-08 那條教訓，位置類 bug 一律先開 log）：暫時加兩行 `MSPY_DEBUG_LOG`，
  把焦點的 context 與量到的矩形印出來，桌面與工作列的答案完全一樣——
  `disabled=0`、`GetTextExt` 回 S_OK、矩形是 `(1919,1031)-(1920,1031)`：寬 1、**高 0**、停在工作區右下角。
  桌面的 `SysListView32` 與工作列的 `Windows.UI.Input.InputSite.WindowClass` 都有 TSF context（要吃 type-ahead），
  所以 `KEYBOARD_DISABLED`／`EMPTYCONTEXT` 兩個旗標都攔不到；舊的守衛寫的是「寬或高其中一個不為 0」，寬度 1 就過關。
  修法：**游標矩形必須有高度**（`bottom > top`），`_FlashModeIndicatorAt` 與 `SystemCaretPoint` 各一處。
  macOS 不必改——`ArtModeHUD` 用 `NSIsEmptyRect`，寬或高其中一個是 0 就不顯示，本來就比 Windows 嚴。
  驗證：加了 log 的建置實測，桌面／工作列 `show=0`、真的文字區（主控台，矩形 `(120,135)-(200,151)`）`show=1`；
  拆掉 log 後重建兩架構、`ctest` x64／x86 各 **178 全過**，並以列舉 `MspyModeIndicator` 視窗的方式確認
  出貨建置在桌面／工作列取得焦點的 5 秒內**沒有任何氣泡視窗出現**。已 `make-package.ps1` 打包並 `install.ps1` 裝到本機
  （兩架構 sha256 與產物相同、explorer 已重啟才會吃到新 DLL；**其他還開著的程式要重開**）。
  尚未發佈：等使用者實測後才 push／打 tag。
- 2026-09-09：**中英氣泡的兩個缺陷（使用者在 Chrome 回報），VERSION 升到 0.8.6**。兩個都只在 `ime/`，
  ...
  **教訓：位置這種「看起來像對的地方」的 bug，不要用推論修**——兩次猜測都自洽、都錯，
  ...
  (c) **氣泡的定位規則收斂成一句話（使用者裁示）：「氣泡是游標的標籤，標得到就顯示，標不到就不顯示」。**
  中間走過一次錯誤的中繼版本（v0.8.6）：那版把「300ms 內答案沒變」當成「不可信」而改用滑鼠位置，
  結果**記事本切分頁時氣泡跑到滑鼠旁**——答案沒變有兩種可能，我把「宿主一開始就答對、沒什麼好修正」
  跟「宿主答的是舊值又不更新」混為一談了。修正後：答案穩定＝就是它；**滑鼠備援整個移除**，
  位置只剩兩個來源（宿主回報的游標矩形、以及**屬於焦點視窗**的系統 caret），兩個都沒有就不顯示。
  程式碼因此變簡單（`_FlashModeIndicatorAt` 的 `pointerFirst` 參數一併消失）。
  **macOS 同步**：`ArtModeHUD` 的滑鼠備援也移除，選單切換（無 client 故無游標）不再顯示氣泡。
  **v0.8.6 發佈後才發現這個退步，該版 release 與 tag 已刪除**，內容併入 v0.8.7。
  (d) 驗證：使用者實測確認三項都修好。core `ctest` x64／x86 各 **178 全過**、check-tutorials 12 課全綠、
  check-drill-coverage 400＋11＝411、check-parity aligned（改動只在兩邊的外殼，共用的 `core/`／`engine/` 零改動）。
  兩架構 DLL 重建並已裝到本機。
...
- 2026-09-08：**四項使用者回報的修正，VERSION 升到 0.8.4**。兩項 Windows、兩項 macOS。
  ...
  (b) **Windows 新增中英提示氣泡**（`ime\SampleIME\ModeIndicator.{h,cpp}`，約 340 行）：
  在游標旁閃「中」／「英」，0.55 秒後淡出，配色沿用候選窗的 `CANDWND_*`。
  **兩個時機**：Shift 單獨輕按，以及焦點落進可輸入的文字區（per-app 記憶把模式換掉、
  卻沒有任何畫面提示的那一刻）。**第一版只做後者**，理由是「切換那次 Windows 自己會跳指示器，
  做了會變成兩個氣泡」——**使用者實測 Win 11 26200 根本沒有那個指示器**，沒有東西可以讓，
  同日補上 Shift 那一條（`_HandleShiftTap` 本來就在 edit session 裡，直接用手上的 cookie 量位置）。
  尺寸同日調小成 28×24 px @96dpi、11pt、ClearType、CS_DROPSHADOW（第一版 40×34、15pt 太大）。
  **macOS 的 `ArtModeHUD` 同日跟上**（使用者要求兩邊一致）：44×44／24pt → 30×26／15pt、
  層級改成 `CGShieldingWindowLevel()`（與候選窗同一個理由）、讀不到游標時從「螢幕中央」
  改成「滑鼠旁邊」，並**補上 `-activateServer:` 的那一次**——那邊原本只在切換時顯示，
  理由寫的是「每次換 app 都閃會很吵」，剛好講反了。
  **兩邊都修掉「會先閃到上一次的字」**：卡片還在畫面上時直接移動／改大小，系統會先把舊內容
  搬到新位置、之後才重畫。作法是先收掉視窗 → 改幾何 → 同步重畫 → 才顯示
  （Windows 另外先把 alpha 壓到 0 再拉回 255）。macOS 另加世代計數，
  否則新的一次閃爍會被上一次還在跑的淡出動畫一起淡掉。
