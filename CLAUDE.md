...
- **中英提示氣泡（2026-09-08）**：在游標旁邊閃一張小卡片顯示「中」或「英」，0.55 秒後淡出。**兩個平台同一個東西、同兩個時機**：Shift 單獨輕按，以及焦點落進可輸入的文字區（per-app 記憶換掉模式卻沒有任何提示的那一刻）。**沒有系統指示器可借**——macOS 對輸入法的內部切換不顯示任何東西，Windows（實測 Win 11 26200）也不顯示。Windows 是 `ime\SampleIME\ModeIndicator.{h,cpp}`（28×24 px @96dpi、11pt、ClearType、`CS_DROPSHADOW`，位置取自選取範圍的 `GetTextExt`，讀不到退回系統 caret 再退回滑鼠）；macOS 是 `mac/src/ArtModeHUD.mm`（30×26 pt、15pt，讀不到直接退回滑鼠）。**重閃時一律先收掉視窗、同步重畫再顯示**，否則會先看到上一次的字。**焦點那次只量一次、答案與上次相同就不顯示（2026-09-09，取代原本最久 570 ms 的輪詢）**；**游標矩形必須有高度才算數（2026-09-09）**：寬 0 正常（游標本來就是一條線），高 0 不是游標——桌面的 `SysListView32` 與 Win11 工作列的 InputSite 都吃 type-ahead、都不標 keyboard-disabled，`GetTextExt` 照樣回 S_OK，回的是工作區右下角一個高度 0 的假矩形。macOS 的 `NSIsEmptyRect` 早就擋掉這一類。
...
- 2026-09-09：**焦點氣泡不再等待，改成「一次量測＋回音判斷」（使用者裁示）**。
  舊版為了 Chromium 會回舊座標，做了 30/80/160/300 ms 的輪詢等它改口——結果**答得對的宿主反而最慢**：
  答案從頭到尾一樣，只能四次等完（**約 570 ms**）才敢採信。使用者回報「有時候會停頓一下」就是這個。
  改法：**只量一次**，若答案跟這個 TIP 實例上次量到的矩形**完全相同**（`_lastCaretRc`），就當它是舊值、
  **什麼都不顯示**；不同就立刻顯示。TIP 每個程式一份，所以「上次量到的」必定是同一個程式的，
  Chromium 的舊答案正好就等於它。`_lastCaretRc` 由 `_FlashModeIndicatorAt` 在接受宿主矩形時更新，
  所以 Shift 那條可信路徑也會把它推到最新（修掉「先打字再切欄位」會誤判成新值的漏洞）。
  刪掉整組延遲機制（`_BeginDeferredFlash`／`kFlashRetryMs`／`SetTimer`＋`thread_local` timer map，約 130 行）。
  **已知代價（使用者知情）**：(a) Chromium 系點進新欄位時可能**不顯示**（宿主還沒改口）；
  (b) 乖乖回答的宿主若**游標位置與上次完全相同**（例如空白記事本點掉再點回同一處）也會被當成回音而不顯示。
  兩者都只是「少一次提示」，不會指錯地方——這是使用者定的取捨順序。
  驗證：合成點擊實測，桌面／工作列**無氣泡**；點桌面再點進記事本，**約 100 ms**（扣掉點擊函式自身的 120 ms 延時）
  就出現在游標旁，舊版是 570 ms。`ctest` x64／x86 各 178 全過，兩架構重建、打包並 install.ps1 裝到本機。
