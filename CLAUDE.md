...
- **中英提示氣泡（2026-09-08）**：在游標旁邊閃一張小卡片顯示「中」或「英」（0.55 秒後淡出，28×24 px @96dpi、11pt、ClearType、CS_DROPSHADOW）。**兩個時機**：Shift 單獨輕按（`_HandleShiftTap`）與焦點落進可輸入的文字區（`ThreadMgrEventSink::OnSetFocus`，per-app 記憶換掉模式卻沒有任何提示的那一刻）。實作 `ime\SampleIME\ModeIndicator.{h,cpp}`，位置取自選取範圍的 `GetTextExt`（Shift 那條直接用手上的 edit cookie，焦點那條自己排唯讀 edit session），讀不到時退回系統 caret、再退回滑鼠位置。macOS 早就有對應的 `ArtModeHUD`。
