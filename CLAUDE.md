...
- **數字鍵兩種身分（2026-08-08 簡化，2026-08-14 換掉閒置那一半）**：畫面上還是注音（未定案）＝全部是聲調鍵；已定案＝`8` 開選單、`9`/`0` 移游標、其餘吃掉。
- **閒置編輯層（2026-08-14）**：沒有組字串時**整排數字**代送編輯鍵——`1` Home、`2` End、`3`/`4` Shift+Home/End、`5` Delete、`6` Backspace、`7`/`8` ↑↓、`9`/`0` ←→。**中英模式共用同一層**（英文模式下組字串非空時數字＝字面數字，`user123` 打得完）。**只吃沒按 Shift 的數字**（Shift+9＝（、Shift+1＝！照舊）；**按著 Ctrl/Alt 一律不接手**。
