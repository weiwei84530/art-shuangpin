...
- **中英提示氣泡（2026-09-08）**：在游標旁邊閃一張小卡片顯示「中」或「英」，0.55 秒後淡出。**兩個平台同一個東西、同兩個時機**：Shift 單獨輕按，以及焦點落進可輸入的文字區（per-app 記憶換掉模式卻沒有任何提示的那一刻）。**沒有系統指示器可借**——macOS 對輸入法的內部切換不顯示任何東西，Windows（實測 Win 11 26200）也不顯示。Windows 是 `ime\SampleIME\ModeIndicator.{h,cpp}`（28×24 px @96dpi、11pt、ClearType、`CS_DROPSHADOW`，位置取自選取範圍的 `GetTextExt`，讀不到退回系統 caret 再退回滑鼠）；macOS 是 `mac/src/ArtModeHUD.mm`（30×26 pt、15pt，讀不到直接退回滑鼠）。**重閃時一律先收掉視窗、同步重畫再顯示**，否則會先看到上一次的字。
