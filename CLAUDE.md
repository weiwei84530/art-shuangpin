...
- **單鍵音節（2026-08-08，2026-08-09 擴及全部 26 個字母）**：省略韻母鍵，聲調鍵／空白直接成字。兩類：注音本身就是音節（`z c s r v i u`＝ㄗㄘㄙㄖㄓㄔㄕ、`y w`＝ㄧㄨ、`a e o`＝ㄚㄜㄛ），與**注音的呼名**（`b p m f`＝ㄅㄛㄆㄛㄇㄛㄈㄛ、`d t n l g k h`＝ㄉㄜ…ㄏㄜ、`j q x`＝ㄐㄧㄑㄧㄒㄧ）。字＝`z4`、是＝`u4`、知＝`v`＋空白、**的＝`d`＋空白、了＝`l`＋空白**；仍可接韻母鍵。實作是「省略＝幫你按預設韻母鍵」（`DecodeSingleKey` 轉呼 `DecodeKeyPair`），故 `d` 與 `de` 同讀音、不可能有歧義。`y` 維持 ㄧ（詞庫統計 ㄧ 系是 ㄩ 系的 4.1 倍），ㄩ 仍是 `yu`／`yy`＋調。**沒有 ㄅㄜ 這個音節**（詞庫 0 筆），ㄅㄆㄇㄈ 呼名帶 ㄛ。
...
- 2026-08-10：**鍵盤標註重畫＋看打練習一句一行**。四件事，全在 `web\` 與 `cli\drill_gen.cpp`。
  ...
  ㄗㄘㄙㄖㄓㄔㄕ 本身就是音節故不印。鍵帽單位 `--u` 50→54px 才塞得下最多四行（`d` 鍵：ㄉ/ㄜ + ㄧㄤ/ㄨㄤ）。
