...
- **`Tab`／`-`／`=` 全部還給應用程式（2026-08-14）**：Tab 不再是 Backspace（由 `6` 接手），`-`/`=` 不再是 Home/End（由 `1`/`2` 接手）也不再跳組字串頭尾。
- **中文模式只輸出全形（2026-08-14）**：沒有全形對映的符號鍵（`@ # $ % & * | - = +`）**閒置時也不輸出**，一律吃掉；要打就 Shift 切英文。`/`＝、（對齊小狼毫，與 `\` 同義但近得多）。
...
- 2026-08-14：**閒置編輯層的第二輪調整＋「那樣」讀音修正**（同日回饋）。
  ...
  **真正的 Delete 鍵在組字中仍然放行給應用程式**，兩邊都是，要修就一起修。
  ...
  （〈九、收尾〉加「你想去哪裡？」與「不要那樣講。」），並把「那樣」寫進 `drillsvoid-words.txt`
  ——手寫課文已覆蓋 ㄋㄚˇ 之後補完就不會再碰它。**坑**：`？` 是句子結束符，寫在一行中間會讓
  drill_gen 報「hand-written line does not convert cleanly」，要拆成兩行。
