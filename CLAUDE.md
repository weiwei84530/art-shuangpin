...
- **Backspace／`6` 在注音階段一次退一個鍵（2026-08-17）**：`bc` 顯示 ㄅㄧㄠ，按一次剩 ㄅ。以前只有「拼不出字的音節」（`nc`）是這樣，能轉成字的（ㄅㄧㄠ 已以一聲進詞格）會整個消失。實作：`Unsettled` 多存 `keys`，Backspace 先刪詞格節點、再把少最後一鍵的部分餵回 `pending_`。
...
- 2026-08-17：**注音階段的四項調整（使用者要求，尚未發佈，VERSION 仍 0.8.1）**。
  ...
  (a) **Backspace 一次退一個鍵**：`bc` 顯示 ㄅㄧㄠ，以前按一次**整個消失**——因為有一聲讀音的
  音節會被 eager 插進詞格（那正是整句轉換能邊打邊修的原因），刪的是整個節點。
  只有「拼不出字」的音節（`nc`＝ㄋㄧㄠ，還留在 `pending_`）才逐鍵退，**同一個畫面兩種行為**。
  修法：`Unsettled` 多存 `keys`（拼出它的原始按鍵），Backspace 先拿掉詞格節點、
  再把少最後一鍵的部分重新餵回 `pending_`。
