...
- **最新音節維持注音顯示（2026-08-02）**：第 2 鍵落下不顯示轉換字（`hk`→ㄏㄠ）；聲調數字／空白／下一音節首鍵／移游標／開選單才定案成字。
- 模態選字（2026-07-27 大改版）：組字中行內整句轉換；**數字全為控制鍵**——`8` 開選單、`9`/`0` 移游標（兩端環繞、游標右字反白錨點）、選單內 `1`-`6` 選字（每頁 6）、`7`/`8` 翻頁不環繞、其他鍵關窗並執行原功能；方向鍵在組字中吃掉無作用。**選定後游標跳過該詞段**（可連按 `8` 一路往右改）。
...
- **空白鍵＝定案不上屏（2026-08-02）**：有待定音節＝以一聲/輕聲預設轉成字、純注音則定案成注音符號（皆留在組字串）；沒東西可定案時才整段 commit（同 Enter）。`,`/`.` 直接上屏「，」「。」。
