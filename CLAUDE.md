...
- 2026-08-13：**ㄧ／ㄨ 音節的母音式拼法＋看打練習三項回饋**。使用者從微軟雙拼過來，
  打「也」的直覺是 ㄧ＋ㄝ＝`yx`（母音優先），而不是 ye 的 `ye`，並問「還有哪些類似的拼法打不出來」。
  (1) **全面盤點後補上 13 組**：規則是「零聲母的 ㄧ／ㄨ 音節，也可以用它**接在聲母後面時**的韻母鍵」——
  `yw yx yc yq ym yd`（ㄧㄚ ㄧㄝ ㄧㄠ ㄧㄡ ㄧㄢ ㄧㄤ）與 `ww wy wv wr wp wd ws`
  （ㄨㄚ ㄨㄞ ㄨㄟ ㄨㄢ ㄨㄣ ㄨㄤ ㄨㄥ）。這 13 組**原本全是死鍵**，所以並存不犧牲任何東西；
  ㄧ `yi`、ㄧㄣ `yn`、ㄧㄥ `y;`、ㄨ `wu`、ㄨㄛ `wo` 與**整個 ㄩ 系**（`yu`/`yy`/`yt`/`yv`/`yr`/`yp`/`ys`）
  兩種算法本來就是同一顆鍵，故無新增。零聲母 a／e／o 三個首鍵**盤點後確認已經完整**（12 個獨立韻母全通）。
  實作放在與 `YodZhuyin`／`WauZhuyin` **分開**的 `YodMedialZhuyin`／`WauMedialZhuyin`，
  只在拼音式查不到時回退——分開的理由是新增的 `IsAlternateKeyPair()`：`KeysForSyllable`
  （看打練習的鍵序、教學稽核的「最省鍵」）**必須跳過母音式**，否則字母順序會讓 ㄨㄟ 被開成 `wv`、
  ㄧㄠ 被開成 `yc`、ㄧㄤ `yd`、ㄨㄤ `wd`，教出去的就不是微軟雙拼的拼法了（兩種都是 2 鍵，
  所以「最省鍵」檢查不會因此報錯，這是**只靠稽核抓不到**的一類錯）。docs/spec.md §4.1、
  docs/guide.md §3.5 已寫入；稽核的有效雙鍵 431→444 組，**可拼出的音節維持 411**（新拼法沒有新音節）。
  (2) **看打第九課結尾的「拽」改成「跩」**（ㄓㄨㄞˋ→ㄓㄨㄞˇ，鍵序 `vy4`→`vy3`）：使用者回報
  「他走路很拽」是對岸用法、打起來別扭。ㄓㄨㄞ 這個音節是 2026-08-10 為了鍵位覆蓋才手寫進去的，
  跩 同樣落在 ㄓㄨㄞ，覆蓋率不受影響（411 全覆蓋）。
  (3) **練完一課用鍵盤繼續**：`R` 重來、`N` 下一課（最後一課不給 `N`，收尾訊息也不會提）。
  判定放在 `drill.shortcut()`，只有 `finished` 時才吃鍵，所以練習中的 R／N 仍然是打錯（會響）。
  N 走 `selectDrill()`，與側邊欄點擊同一條路徑，選取狀態才會跟著移動。
  (4) **鍵盤三段式切換**（鍵盤下方按鈕）：0 提示下一鍵／1 不提示鍵位／2 隱藏鍵盤，記在
  `localStorage['drillKeyboard']`。**只在看打練習出現**——教學課程的亮鍵是課文本身不是提示，
  關掉等於把課廢掉。mode 1 只擋 `showHint()` 的鍵帽高亮（打對時的 `pressed` 動畫保留，
  那是回饋不是答案；「下一鍵：X」那行文字也保留）。隱藏時 `.kb-stage` 收掉 `flex:1`，
  按鈕才不會孤零零留在畫面正中間。**坑**：按鈕在 `.kb-stage` 裡，而 stage 的 pointerdown
  會 `setPointerCapture` 做旋轉拖曳，會把 click 吃掉——pointerdown 要先擋掉 `.kb-foot` 內的目標。
  驗證：`ctest` 174 全過（+1）、check-drill-coverage 411 全覆蓋、check-tutorials 12 課全綠、
  Chrome 實測（三段切換、隱藏鍵盤下仍可打完第九課、R／N、重新整理後設定還在）無 console 錯誤；
  x64/x86 DLL 已重建。
