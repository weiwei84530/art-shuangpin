...
- **中英提示氣泡（2026-09-08）**：在游標旁邊閃一張小卡片顯示「中」或「英」，0.55 秒後淡出。**兩個平台同一個東西、同兩個時機**：Shift 單獨輕按，以及焦點落進可輸入的文字區（per-app 記憶換掉模式卻沒有任何提示的那一刻）。**沒有系統指示器可借**——macOS 對輸入法的內部切換不顯示任何東西，Windows（實測 Win 11 26200）也不顯示。Windows 是 `ime\SampleIME\ModeIndicator.{h,cpp}`（28×24 px @96dpi、11pt、ClearType、`CS_DROPSHADOW`，位置取自選取範圍的 `GetTextExt`，讀不到退回系統 caret 再退回滑鼠）；macOS 是 `mac/src/ArtModeHUD.mm`（30×26 pt、15pt，讀不到直接退回滑鼠）。**重閃時一律先收掉視窗、同步重畫再顯示**，否則會先看到上一次的字。**焦點那次先量一次，答案是新的就立刻顯示（2026-09-09）**——只有答案跟這個 TIP 實例上次量到的矩形完全相同（＝宿主回的是舊值）才照舊等它改口（30/80/160/300 ms，等滿就採信該答案）；**游標矩形必須有高度才算數（2026-09-09）**：寬 0 正常（游標本來就是一條線），高 0 不是游標——桌面的 `SysListView32` 與 Win11 工作列的 InputSite 都吃 type-ahead、都不標 keyboard-disabled，`GetTextExt` 照樣回 S_OK，回的是工作區右下角一個高度 0 的假矩形。macOS 的 `NSIsEmptyRect` 早就擋掉這一類。
...
- 2026-09-09：**焦點氣泡的等待改成「只在需要時才等」（使用者裁示）**。
  舊版為了 Chromium 會回舊座標，一律做 30/80/160/300 ms 的輪詢等它改口——結果**答得對的宿主反而最慢**：
  答案從頭到尾一樣，只能四次等完（**約 570 ms**）才敢採信。使用者回報「有時候會停頓一下」就是這個。
  改法：先量一次，**答案是新的就立刻顯示**；只有當答案跟這個 TIP 實例上次量到的矩形**完全相同**
  （`_lastCaretRc`，由 `_FlashModeIndicatorAt` 在接受宿主矩形時更新，所以 Shift 那條可信路徑也會推新它）
  才判定成「宿主回舊值」，這時**照舊走原本的輪詢**，等到答案改變就顯示、等滿了就採信原答案
  （＝焦點回到同一個欄位，游標本來就在那）。TIP 每個程式一份，所以「上次量到的」必定是同一個程式的，
  Chromium 的舊答案正好等於它。記事本、終端機這種每次都答得對的宿主從此**零等待**。
  **同日先做過一個更激進的版本並由使用者撤銷（勿再實作）**：那版偵測到回音就**直接不顯示**、
  整組延遲機制刪光，代價是 Chromium 系點進新欄位時氣泡整個消失——實測後使用者裁示不可接受，
  提示消失比慢半拍糟。現在的版本只把「不必等的情況」變快，會等的那半條路徑行為與先前完全相同。
  驗證：兩架構重建、`ctest` x64／x86 各 178 全過，打包並 install.ps1 裝到本機；合成點擊實測
  桌面／工作列仍無氣泡，點進記事本立刻出現。
